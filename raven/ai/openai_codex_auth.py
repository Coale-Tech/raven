...
must go to the ChatGPT backend, not api.openai.com. See `providers.py`.

Vendored from insights/ai/openai_codex_auth.py, retargeted to Raven Settings.
"""
...
PENDING_CACHE_KEY = "raven:openai_codex_device_login"
...
def _guard():
	...
	frappe.has_permission("Raven Settings", "write", throw=True)
...
def _stored_value(fieldname: str) -> str:
	"""Read a stored Raven Settings field back as plain text.
	...
	"""
	return str(frappe.db.get_single_value("Raven Settings", fieldname) or "")


def _persist(tokens: Dict[str, Any]) -> Dict[str, str]:
	"""Store a freshly minted credential on Raven Settings."""
	...
	# The token endpoint never returns `expires_in` -- the access token's own
	# `exp` claim is the only real expiry source. Fall back to a conservative
	# 1h assumption only if that claim is somehow missing.
	...
	set_value("Raven Settings", "openai_oauth_access_token", access)
	if refresh:
		set_value("Raven Settings", "openai_oauth_refresh_token", refresh)
	set_value("Raven Settings", "openai_oauth_account_id", identity["account_id"])
	set_value("Raven Settings", "openai_oauth_expires_at", expires_at)
	...
	set_value("Raven Settings", "openai_oauth_account_label", label)
	# Connecting a subscription is the intent to use it.
	set_value("Raven Settings", "enable_chatgpt_subscription", 1)
	return identity
...
def _clear_credential():
	...
	for field in (
		...
		frappe.db.set_single_value("Raven Settings", field, "")
	frappe.db.set_single_value("Raven Settings", "openai_oauth_expires_at", 0)
	frappe.db.set_single_value("Raven Settings", "enable_chatgpt_subscription", 0)
...
def get_access_token(settings=None) -> Optional[str]:
	...
	settings = settings or frappe.get_single("Raven Settings")
	...
	if expires_at and expires_at - REFRESH_SKEW <= int(time.time()):
		...
		if frappe.db.get_single_value("Raven Settings", "openai_oauth_access_token"):
			return token
		return None
	return token
